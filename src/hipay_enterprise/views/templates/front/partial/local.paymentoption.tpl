        ...
        <form class="localpayment" method="post" action="{$paymentProduct.link|escape:'html':'UTF-8'}">
            <p class="payment_module" id="hipay_payment_button">
                <a {if (isset($paymentProduct.errorMsg) && empty($paymentProduct.errorMsg)) || (!isset($paymentProduct.errorMsg))}href="javascript:void(0);" onclick="{literal}$(this).closest('form').submit();{/literal}"{/if}
                   title="{l s='Pay by ' mod='hipay_enterprise' } {$productName|escape:'htmlall':'UTF-8'}"

                   style="padding-left: 13px;">
                    <img src="{$domain|cat:$paymentProduct.payment_button|escape:'html':'UTF-8'}"
                         style="max-width: 160px;max-height: 50px;" alt="{$productName|escape:'htmlall':'UTF-8'}"/>
                    {l s='Pay by ' mod='hipay_enterprise' } {$productName|escape:'htmlall':'UTF-8'}
                    ...
                    <div id="error-js-oc" style="" class="alert alert-danger">
                        <ol>
                            {foreach $paymentProduct.errorMsg as $msg}
                                <li class="error-oc">{$msg|escape:'htmlall':'UTF-8'}</li>
                            {/foreach}
                        </ol>
                    </div>
                {/if}
